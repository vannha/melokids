<?php
if(!class_exists('WPH_Widget') || !class_exists('EF4Framework')) return;
/**
 * Price filter step
 *
 */
...
if ( ! class_exists( 'MeloKids_Widget_Price_Filter' ) ) {
	...
	add_action( 'widgets_init', 'load_melokids_price_filter_widget' );

	function load_melokids_price_filter_widget() {
		register_ef4_widget( 'MeloKids_Widget_Price_Filter', 1 );
	}

	class MeloKids_Widget_Price_Filter extends WPH_Widget {
		...
		function __construct() {
			...
			$args = array(
				'slug'        => 'zk_price_filter',
				// Widget Backend label
				'label'       => esc_html__( '[MeloKids] Price filter', 'melokids' ),
				// Widget Backend Description
				'description' => esc_html__( 'Price filter list', 'melokids' ),
			);
			...
			$args['fields'] = array(
				array(
					'id'   => 'title',
					'type' => 'text',
					'std'  => esc_html__( 'Price filter', 'melokids' ),
					'name' => esc_html__( 'Title', 'melokids' ),
				),
			);
				...
				<div class="melokids-price-filter">
					<ul>
						<?php foreach ( $links as $link ) : ?>
							<li class="zk-price-filter <?php echo esc_attr( $link['class'] ); ?>">
								<a href="<?php echo esc_url( $link['href'] ); ?>"
								   class="wc-widget-filter <?php echo esc_attr( $link['class'] ); ?>"><?php echo  melokids_allow_html($link['title']) ; ?></a>
							</li>
						<?php endforeach; ?>
					</ul>
				</div>
		...
		private function generate_price_links( $min, $max, $min_price, $max_price ) {
			...
			$steps = apply_filters( 'melokids_price_filter_steps', 5 );
