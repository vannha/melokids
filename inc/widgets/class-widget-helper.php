...
if ( ! class_exists( 'WPH_Widget' ) ) {
    ...
    class WPH_Widget extends WP_Widget {
        ...
        function create_widget( $args ) {
            ...
            // check options
            $this->options = array( 'description' => $description );
            if ( ! empty( $options ) ) {
        ...
        function create_field_select_group( $key, $out = "" ) {
            ...
            foreach ( $key['groups'] as $group => $fields ) {
        ...
        function create_field_radio( $key, $out = "" ) {
            ...
            foreach ( $key['options'] as $field => $option ) {
                ...
                $out .= '<label><input class="radio" type="radio" id="' . esc_attr( $key['_id'] ) . '" name="' . esc_attr( $key['_name'] ) . '" value="' . esc_attr( $option['value'] ) . '" ';

                if ( esc_attr( $selected ) == esc_attr( $option['value'] ) ) {
                    $out .= ' checked="checked" ';
                }
        ...
        function create_field_posttype( $key, $out = "" ) {
            ...
            foreach ( $posttypes as $posttype ) {
                ...
                if ( ( $posttype->name == 'attachment' ) or ( $posttype->name == 'options' ) ) {
                    continue;
                }

                $out .= '<option value="' . asc_attr( $posttype->name ) . '" ';
        ...
        function create_field_taxonomy( $key, $out = "" ) {
            ...
            foreach ( $taxes as $tax ) {
                ...
                foreach ( $posttypes_obj as $posttype_obj => $posttype ) {
                    $out .= '<option value="' . asc_attr( $taxonomy->name ) . '" ';
        ...
        function create_field_taxonomyterm( $key, $out = "" ) {
            ...
            foreach ( $taxes as $tax ) {
                ...
                $out .= '>' . esc_html_e( 'Any Categories', 'melokids' ) . '</option>';

                foreach ( $terms as $term ) {

                    //make array as pattern ( $term->taxonomy , $term->name);
                    $out .= '<option value="' . asc_attr( $term->slug ) . '" ';
        ...
        function create_field_pages( $key, $out = "" ) {
            ...
            foreach ( $pages as $page ) {
                ...
                $out .= '<option value="' . asc_attr( $page->ID ) . '" ';
